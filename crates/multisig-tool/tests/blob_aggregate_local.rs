...
//! (see multisig-tool README / dusk-vm-issue-1). Live testnet uses secure signing.
...
use multisig_encoding::PartialSig;
use multisig_tool::blob::{
...
fn file_byo_two_of_three_aggregate_verifies_locally() {
    ...
    for (sk, pk) in [(&sk1, &pk1), (&sk2, &pk2)] {
        let digest = multisig_encoding::gate_blob_for_signing(&blob).expect("gate");
