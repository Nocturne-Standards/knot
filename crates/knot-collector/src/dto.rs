...
//!
//! Deliberately **duplicated** (not shared via `knot-encoding`) — that
//! crate is `no_std` + Apache-2.0 and consumed by a WASM contract
//! (`knot-proposals`); teaching it JSON/hex would bloat a size-sensitive
...
/// BLS public key length in bytes (96) — matches `knot-tool`'s `hex96`.
pub const PK_BYTES: usize = 96;
/// `signed_digest` length in bytes (32) — matches `knot-tool`'s `hex32`.
pub const DIGEST_BYTES: usize = 32;

/// Outer blob discriminator. Missing `kind` deserializes as
/// [`BlobKind::Proposals`] (v1 compatibility). Lockstep with
/// `knot-tool::blob::BlobKind`.
