//! End-to-end roundtrip against a *real* `knot-collector` HTTP server
//! (spawned as its own OS process, never linked in — `knot-tool` must
//! never add `knot-collector` as a Cargo dependency) exercising
...
fn workspace_root() -> PathBuf {
    // CARGO_MANIFEST_DIR = <workspace>/crates/knot-tool
    Path::new(env!("CARGO_MANIFEST_DIR"))
        .parent()
        .and_then(Path::parent)
        .expect("knot-tool sits two levels under the workspace root")
        .to_path_buf()
}
...
/// Builds (if needed) and spawns the `knot-collector` binary bound to
...
fn spawn_collector(bind: &str, db_path: &Path) -> CollectorProcess {
    let build = Command::new("cargo")
        .args(["build", "-p", "knot-collector"])
        .current_dir(workspace_root())
        .status()
        .expect("run cargo build -p knot-collector");
    ...
    let child = Command::new(&bin)
        ...
        .env("KNOT_COLLECTOR_DB", db_path)
        .stdout(Stdio::null())
        .stderr(Stdio::null())
        .spawn()
        .expect("spawn knot-collector");
...
async fn two_of_three_push_sign_sign_pull_aggregate_roundtrip() {
    ...
    let db_path = std::env::temp_dir().join(format!("knot-collector-roundtrip-{port}.sqlite"));
