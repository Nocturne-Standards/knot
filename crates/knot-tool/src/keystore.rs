...
//! user signs knot-registry quorum messages with. Deliberately NOT
...
pub fn default_path() -> PathBuf {
    ...
    Path::new(&home).join(".knot").join("identities.dat")
}

/// Default store path, falling back to pre-rename locations for one release
/// when the new path is absent (`~/.knot-tool/`, then `~/.multisig-tool/`).
pub fn resolve_default_path() -> PathBuf {
    ...
    for legacy_dir in [".knot-tool", ".multisig-tool"] {
        ...
        .map_err(|_| {
            anyhow::anyhow!(
                "wrong password, or identity store is corrupt (stores encrypted under the old \
                 100k-round KDF must be re-created — delete ~/.knot/identities.dat or \
