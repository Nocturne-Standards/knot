//! Tests for `multisig-registry`: account creation/threshold validation,
...
const MULTISIG_REGISTRY_BYTECODE: &[u8] = include_bytes!(
...
fn initialize() -> Session {
    ...
    session
        .deploy(
            MULTISIG_REGISTRY_BYTECODE,
            ContractData::builder()
                .owner([0; 32])
                .contract_id(MULTISIG_REGISTRY_ID),
            POINT_LIMIT,
        )
...
fn create_account_rejects_out_of_range_threshold() {
    ...
    let result =
        session.call::<CreateAccountArgs, u64>(MULTISIG_REGISTRY_ID, "create_account", &args, POINT_LIMIT);
...
fn create_account_then_query_roundtrips() {
    ...
    let id = session
        .call::<CreateAccountArgs, u64>(MULTISIG_REGISTRY_ID, "create_account", &args, POINT_LIMIT)
        .expect("create_account should succeed")
        .data;

    let view = session
        .call::<u64, Option<MultisigAccountView>>(MULTISIG_REGISTRY_ID, "account", &id, POINT_LIMIT)
...
fn verify_quorum_true_only_with_enough_distinct_valid_signers() {
    ...
    let id = session
        .call::<CreateAccountArgs, u64>(MULTISIG_REGISTRY_ID, "create_account", &create_args, POINT_LIMIT)
        .expect("create_account should succeed")
        .data;
    ...
    let passed = session
        .call::<VerifyQuorumArgs, bool>(MULTISIG_REGISTRY_ID, "verify_quorum", &args, POINT_LIMIT)
...
fn change_account_requires_quorum_and_bumps_nonce_preventing_replay() {
    ...
    session
        .call::<ChangeAccountArgs, ()>(MULTISIG_REGISTRY_ID, "change_account", &good_args, POINT_LIMIT)
...
fn wire_option_none_is_32_zero_bytes() {
    ...
    // contract/query state failure — see multisig-tool README.
...
fn diagnostic_reads_and_diagnose_quorum_roundtrip() {
    ...
    assert_eq!(
        session
            .call::<(), u64>(MULTISIG_REGISTRY_ID, "next_account_id", &(), POINT_LIMIT)
    ...
    let meta = session
        .call::<u64, Option<AccountMeta>>(MULTISIG_REGISTRY_ID, "account_meta", &id, POINT_LIMIT)
        .unwrap()
        .data
        .expect("meta");
    ...
    let keys = session
        .call::<u64, Option<Vec<Vec<u8>>>>(MULTISIG_REGISTRY_ID, "member_key_bytes", &id, POINT_LIMIT)
        .unwrap()
        .data
        .expect("keys");
