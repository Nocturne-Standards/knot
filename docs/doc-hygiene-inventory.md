...
### `crates/multisig-tool/README.md` (423 lines — largest, densest)
...
### `crates/multisig-collector/README.md`

| Section | Category | Notes |
|---|---|---|
| Title, trust-model intro | `keep-public` | Good. |
| §Status — 3 dated bullets, one citing a spec doc | `move-to-docs-internal` | Smaller instance of the same pattern; low cost to trim to current-state only. Spec-doc citation (`docs/superpowers/specs/2026-07-23-multisig-collector-monorepo-demo-design.md`) is itself under `docs/superpowers/` — see that section above (move-to-nocturne-docs / exclude). |
...
### `crates/multisig-encoding/README.md`
...
|---|---|---|
| `../../../docs/multisig/multisig-collector-deploy-runbook.md` | `multisig-tool/README.md` | `add` (write a real runbook here) — see gap map C6. |
| `../../../docs/multisig/testnet-read-lag-2026-07-22.md` | `multisig-tool/README.md` (×2) | `delete-duplicate` — historical incident writeup, low value to a new public reader; drop the citation or fold the one-sentence lesson ("RUES free-reads need raw bodies, not hex") directly into §Known caveats, which already states the lesson. |
| `../../../docs/multisig/multisig-suite-and-atlas-implementation-plan.md` | `multisig-encoding/README.md` | `move-to-nocturne-docs` if the plan still matters externally, else `delete-duplicate`. |
| `../../../docs/versioning.md` | `multisig-tool/README.md`, `multisig-tool/CHANGELOG.md`, `multisig-collector/README.md`, `multisig-collector/CHANGELOG.md` | `add` — see gap map B3; this one has real expected content missing, not just a misplaced pointer. |
| `../../../docs/superpowers/specs/2026-07-31-shared-code/26-multisig-shared-call-types.md` | `multisig-encoding/README.md` | `move-to-nocturne-docs` or inline the one relevant paragraph (Layer-E call-types rationale) directly into the README, since the spec dir itself is excluded from the public tag. |
| `../../../docs/superpowers/specs/2026-07-26-multisig-website-demo-design.md` | `multisig-tool/README.md` | Same as above. |
| `../../../deployments/testnet.json` | `multisig-tool/README.md` (×2) | `move-to-docs-internal` — this is real operator data (which repo's tree it should live in is a monorepo-layout question outside this track's scope); at minimum the README should say where a *standalone* knot checkout gets this file from, since today it implies a file this repo doesn't ship. |
| `../../../prediction-market/docs/council-resolve-testing.md` | `multisig-tool/README.md` (×2) | `delete-duplicate` from knot's side (belongs entirely to the `prediction-market` repo now) — replace with a short "see prediction-market's own docs" pointer with no relative path (a repo name, not a filesystem path, since post-carve they're siblings at best). |
| `../../../prediction-market/crates/pm-admin-tool/README.md`, `.../pm-council-tool/README.md` | `multisig-tool/README.md` | Same as above. |
| `../../../references/repos/multisig-contract` | `multisig-registry/README.md` | `delete-duplicate` — comparison-to-upstream-example note; nice-to-have context, safe to drop the dead link or replace with the public Dusk example's actual public URL if one exists. |
| `../../../references/dusk-native/crate-source-locations.md`, `.../dusk-vm-issue-1-ephemeral-hardfork-policy-unreachable.md` | `multisig-registry/README.md`, `multisig-tool/README.md` (×2), `multisig-tool/src/bls.rs` module doc | `move-to-docs-internal` — these describe a real, still-relevant `dusk-vm` gotcha (`VM::ephemeral()` hardfork policy) that affects how this repo's own tests are written. Worth a short internal note (or upstreaming as a `dusk-vm` issue if not already filed) rather than a dead pointer, since the gotcha is genuinely load-bearing for anyone extending the test suite. |
| `../../../nocturne-lab/`, `bash nocturne-lab/scripts/sync-assets.sh` | `multisig-tool/README.md` | `move-to-docs-internal` — asset-sync tooling for the marketing/demo Lab UI, maintainer-facing only. |
| `references/testnet-wallet.md` (no `../../../` prefix — cited as if repo-root-relative) | `multisig-tool/README.md` (×2), `multisig-tool/src/chain.rs:315,329` (**runtime error text**, not just docs) | See gap map A15 — this is the highest-priority dead link because it surfaces in an actual CLI error message a real user will hit. `add` (vendor the real content) or rewrite the message to be self-contained. |
| `atlas/README.md` | `multisig-tool/README.md` (monitoring note) | `delete-duplicate` from knot's perspective — `atlas/` is explicitly "outside this nest" per the root README's own framing; the monitoring note is good advice but shouldn't cite a path knot doesn't ship. Reword to reference Atlas by name without a filesystem path. |
| `root CLAUDE.md` | `multisig-registry/README.md`, `docs/security-audit-2026-07-28*.md` | `delete-duplicate` — internal AI-agent tooling note (dusk-forge codegen gotcha), doesn't exist here; if the underlying gotcha still matters, fold the one sentence into the README directly instead of citing an agent config file. |
