...
`multisig-encoding` (12 + 5 unit, `layout_goldens.rs` 155 lines),
`multisig-registry` (14 integration tests under `VM::ephemeral()`,
`layout_goldens.rs` 230 lines), `multisig-proposals` (16 integration
tests incl. CEI/reentrancy, `layout_goldens.rs` 183 lines),
`multisig-collector` (11 unit in `store.rs`, 3 `tokio::test` HTTP
integration), `multisig-tool` (`blob.rs` 10, `mock_ledger.rs` 7,
`keystore.rs` 1, `chain.rs` 3, plus 2+1 integration tests). Headline
finding: `multisig-tool/src/rpc.rs` (2011 lines) — the file containing
