...
coordinator/multisig-tool is final trust) and a "Prove" (on-chain verify_quorum /
verify_quorum_aggregate re-derives quorum from live state) integration mode.

Finding (code-verified 2026-08-04): every write path in multisig-registry
(verify_quorum, verify_quorum_aggregate, change_account) and multisig-proposals
...
trust only exists transiently inside multisig-tool, before anything reaches
...
Do not market multisig-tool as providing an independent Coord-style
