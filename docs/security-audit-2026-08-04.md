...
`knot-tool`; the collector is untrusted by design.
...
re-verified by direct code read (file:line cited). `knot-tool` got a full
...
| Sev | Location | Issue |
|---|---|---|
| **Critical** | `knot-tool/src/rpc.rs:1919-1986`, `src/main.rs` PmResolveCmd::Sign | PM council-resolve signing has **no membership gate at all** — not server-side, not even UI-only (worse than the July-28 `pm-council-tool` precedent, which at least disabled a button client-side) |
| **High** | `knot-tool/src/rpc.rs:1988-2011`, `src/main.rs:1367-1386` | PM-resolve submit target (`pm_contract_id`) taken verbatim from the untrusted collector blob, never cross-checked against a live-fetched PM deployment id — stale/redeployed contract → silent wrong-target submit |
| **High** | `knot-tool/src/pm_resolve_types.rs`, `src/pm_read_types.rs` | Cross-**repo** hand-mirrored rkyv ABI types (`ResolveArgs`/`CouncilSigEntry`/`MarketInfo`) for `prediction-market`, "kept in sync by hand" — no compiler enforcement, no golden-vector test, violates repo's shared-code rule; PM lives in a separate repo post-carve so drift is now silent |
| Medium | `knot-tool/src/rpc.rs` (`api_proposal_approve`, `api_quorum_submit`, `api_quorum_agg_submit`, `api_change_account_submit`) + CLI equivalents | No live on-chain membership/threshold pre-check before any generic sign path either — mitigated by robust on-chain re-verification in this repo's own contracts (see Prove-mode confirmation below), but still a TCB-hygiene gap the posture-D brief calls out |
| Medium | `knot-tool/src/rpc.rs:1693-1731` (`api_pm_resolve_init`) | Blob `threshold` is client-supplied with no live cross-check against the registry account's current threshold at creation time; `registry_warn` only checks member PKs, never threshold — PM-flow analogue of July-28's pm-council-tool "no live threshold re-check" Medium |
| Low | `knot-collector/src/store.rs` | No cap on total proposal/party row count (unchanged from July-28) |
| Low | `knot-registry/src/state.rs`, `knot-proposals/src/state.rs` | Unbounded account/proposal count (permissionless by design, unchanged) |
| Low/Info | `knot-encoding/src/lib.rs` | `change_account_digest`/`council_resolve_digest` member-pk ordering is doc-stated, not canonicalized/enforced (unchanged doc gap) |
| **Resolved since 2026-07-28** | `knot-tool/src/keystore.rs:34` | PBKDF2-SHA256 rounds 100k → **600k** — fixed |
| **Resolved since 2026-07-28** | `knot-tool/src/rpc.rs:219-231` | Bearer-token compare `==` → **`subtle::ConstantTimeEq`** — fixed |
| **Resolved since 2026-07-28** | `knot-encoding/src/lib.rs:145-146` | `u32::try_from(...).expect(...)` panic → **`checked_u32_len` returning `Result`** — fixed |
| **Resolved since 2026-07-28** | `knot-registry/src/state.rs` (`next_id`/`nonce`) | Unguarded `+=` → **`checked_add(...).expect(...)`** (hard panic instead of silent wraparound) — fixed |
| **Resolved since 2026-07-28** | `knot-encoding/src/fingerprint.rs:34` | Wordlist-length invariant `debug_assert_eq!` (compiled out in release) → **`assert_eq!`** (always checked) — fixed |

No Critical/High findings survive in `knot-registry`, `knot-proposals`,
or `knot-collector` — all three independently re-verified clean and
consistent with docs (see per-crate sections). All Critical/High findings
this wave are in `knot-tool`, the designated primary TCB under posture D.

---

## Critical — PM council-resolve sign path has zero membership gate

**Location:** `crates/knot-tool/src/rpc.rs:1919-1986` (`api_pm_resolve_sign`);
identical gap in CLI `PmResolveCmd::Sign` (`src/main.rs`, same crate); UI at
`crates/knot-tool/static/pm-resolve-app.js` (`sign()`, lines 294-320) and
...
**Location:** `crates/knot-tool/src/rpc.rs:1988-2011` (`api_pm_resolve_submit`);
`crates/knot-tool/src/main.rs:1367-1386` (`PmResolveCmd::Submit`).
...
in `knot-tool`'s own PM-resolve feature, unfixed.
...
`grep pm_contract_id` across `crates/knot-tool/src` shows no call to
...
**Location:** `crates/knot-tool/src/pm_resolve_types.rs` (`CouncilSigEntry`,
`ResolveArgs`); `crates/knot-tool/src/pm_read_types.rs` (`MarketStatus`,
`MarketInfo`).
...
use `#[path = "../../knot-registry/src/call_types.rs"]` same-repo file
...
analogous to `achievement-nft-types`/`knot-encoding`'s own
`call-types` feature) that both `prediction-market` and `knot`'s
`knot-tool` depend on by version pin; until that lands, add a
golden-vector byte-layout test in `knot-tool` (encode a fixed
...
**Location:** `crates/knot-tool/src/rpc.rs` — `api_proposal_approve`
...
- `knot-registry::verify_quorum`/`verify_quorum_aggregate`
...
- `knot-proposals::approve` (`state.rs:191-225`) panics outright if
  `!view.members.contains(&args.signer)`, before recording the approval.
- `knot-proposals::finalize` (`state.rs:256-326`) re-derives quorum via
...
**Location:** `crates/knot-tool/src/rpc.rs:1693-1731` (`api_pm_resolve_init`).
...
## knot-registry — re-verified clean, three July-28 items now fixed
...
## knot-proposals — re-verified clean
...
## knot-collector — re-verified clean, untrusted-by-design posture intact
...
## knot-encoding — re-verified clean, two July-28 items now fixed
...
correct. `registry_types.rs`/`proposals_types.rs` in `knot-tool` correctly
...
`dusk-network/rusk` public tag pins). Licensing edges clean: `knot-tool`
(Apache-2.0) depends only on Apache/MIT-class crates plus `knot-encoding`
(Apache, path dep) — no Apache crate depends on the AGPL `knot-collector`
(collector is a separate binary reached only over HTTP). `knot-collector`
...
`knot-registry`/`knot-proposals` independently re-verifies
...
for this suite**, and treat `knot-tool`'s own gating (digest recompute,
